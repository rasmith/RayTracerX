...
#include "camera.hpp"
namespace ray {
Camera::Camera() : near_(0.0f), far_(0.0f), aspect_(0.0f), fov_(0.0f),
        at_(0.0f, 0.0f, 0.0f), position_(0.0f, 0.0f, 0.0f),
        up_(0.0f, 0.0f, 0.0f) {
}

Camera::Camera(const Camera& cam)  : near_(cam.near_), far_(cam.far_),
        aspect_(cam.aspect_), fov_(cam.fov_), at_(cam.at_),
        position_(cam.position_), up_(cam.up_) {
...
Camera::Camera(float near, float far, float aspect, float fov,
           ...
           const glm::vec3& up) : near_(near), far_(far), aspect_(aspect),
                    fov_(fov), at_(at), position_(position), up_(up) {
}
...
glm::vec4 Camera::Project(const glm::vec3& point) {
}

glm::vec4 Camera::Unproject(const glm::vec3& point) {
}
